...
impl AdHoc {
    pub async fn run(
        ...
        let (callback_plugin_path_cfg, _) = config_manager
            .lock()
            .await
        ...
        let callback_plugin_paths: Vec<PathBuf> = callback_plugin_path_cfg
            .to_string_lossy()
            .split(':')
        ...
        let mut tqm = TaskQueueManager::new(Some(options.forks as usize), &callback_plugin_paths);
